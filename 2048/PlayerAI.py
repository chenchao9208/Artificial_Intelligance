...
class PlayerAI(BaseAI):
    ...
    def alpha_beta_search(self, grid, alpha, beta, depth):
        result = self.max_value(grid, alpha, beta, depth, 1)
        return result
    ...
    def max_value(self, grid, alpha, beta, depth, flag):
        ...
        for move in available_moves:
            ...
            if flag == 1 and self.need_pruning(grid_moved):
                continue
    ...
    def min_value(self, grid, alpha, beta, depth, flag):
        ...
        for cell in available_cells_2:
            ...
            child = self.max_value(grid_inserted, alpha, beta, depth, flag)
            child_value = child.get("value")
    ...
    # slight pruning for max_value stage apart from alpha-beta pruning
    def need_pruning(self, grid):
        ...
        for i in xrange(4):
            for j in xrange(4):
                cell_list.append({"axis": [i, j], "value": grid.map[i][j]})
        cell_list.sort(key=self.f)
        first_axis = cell_list.pop().get("axis")
        if first_axis != [0, 0] and first_axis != [0, 3] and \
                        first_axis != [3, 0] and first_axis != [3, 3]:
            return True

        second = cell_list.pop()
        second_axis = second.get("axis")
        third = cell_list.pop()
        #third_axis = third.get("axis")
        if second.get("value") != third.get("value") and second_axis[0] != first_axis[0] and second_axis[1] != \
                first_axis[1]:
            return True
        return False
    ...
    def evaluate(self, grid):
        score = self.monotonicity(grid) * monoWeight \
